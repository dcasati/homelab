...
resource "proxmox_virtual_environment_vm" "microk8s_vm" {
  vm_id       = var.vmid
  node_name   = var.proxmox_node_name
  name        = "microk8s-${var.vm_settings.suffix}"
  description = "MicroK8s Node - ${var.vm_settings.suffix}"
  ...
  tags = ["microk8s", var.vm_settings.suffix]
